...
func NewSshClient(host string, port int, username string, password string) (*ssh.Client, error) {
...
func hostKeyCallBackFunc(host string) ssh.HostKeyCallback {
	...
	if err != nil {
		global.GVA_LOG.Error("find known_hosts's home dir failed", zap.Any("err", err))
	}
	file, err := os.Open(hostPath)
	if err != nil {
		global.GVA_LOG.Error("can't find known_host file:", zap.Any("err", err))
	}
	defer file.Close()
	...
	for scanner.Scan() {
		...
		if strings.Contains(fields[0], host) {
			...
			if err != nil {
				global.GVA_LOG.Error("error parsing %q: %v", zap.Any("err parse", fields[2]),
					zap.Any("err", err))
			}
			break
		}
	...
	if hostKey == nil {
		global.GVA_LOG.Error("no hostkey for %s,%v", zap.Any("host", host), zap.Any("err", err))
	}
...
func publicKeyAuthFunc(kPath string) ssh.AuthMethod {
	...
	if err != nil {
		global.GVA_LOG.Error("find key's home dir failed", zap.Any("err", err))
	}
	key, err := ioutil.ReadFile(keyPath)
	if err != nil {
		global.GVA_LOG.Error("ssh key file read failed", zap.Any("err", err))
	}
	// Create the Signer for this private key.
	signer, err := ssh.ParsePrivateKey(key)
	if err != nil {
		global.GVA_LOG.Error("ssh key signer failed", zap.Any("err", err))
	}
